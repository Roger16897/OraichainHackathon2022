...
namespace _1877_HA.Areas.Admin.Controllers
{
    public class ReflectionController
    {
        public List<Type> GetControllers(string namespaces)
        {
            ...
            IEnumerable<Type> types = assembly.GetTypes().Where(type => typeof(Controller).IsAssignableFrom(type) || typeof(ApiController).IsAssignableFrom(type) && type.Namespace.Contains(namespaces)).OrderBy(x => x.Name);
            return types.ToList();
        }
        ...
        public List<string> GetActions(Type controller)
        {
            ...
            {
                if (method.ReflectedType.IsPublic && !method.IsDefined(typeof(System.Web.Http.NonActionAttribute)))
                {
